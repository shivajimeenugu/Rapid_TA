...
console.log(pincode);
    ...
    <script>
        ...
        {
                multi_delete_code='<label for="default-search" class="mb-2 text-sm font-medium text-gray-900 sr-only dark1:text-white">Search</label>';
                multi_delete_code+='<div class="relative">';
                multi_delete_code+='<div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">';
                multi_delete_code+='<svg aria-hidden="true" class="w-5 h-5 text-gray-500 dark1:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>';
                multi_delete_code+='</div>';
                ...
                multi_delete_code+='<button type="submit" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark1:bg-blue-600 dark1:hover:bg-blue-700 dark1:focus:ring-blue-800">Search</button>';
                multi_delete_code+='</div>';
                $('#searchbox').html(multi_delete_code);
        }

        function toggle_selected(id,sno)
        {

            var idx = selected_bills.indexOf(id);
            if(idx != -1)
            {
                $('#billcard_'+id).removeClass().addClass(
                            'sno px-3 py-1 font-extrabold text-white text-sm bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-full  '
                            );
                $('#billcard_'+id).html(sno);

                selected_bills.splice(idx, 1)


            }
            else{
                $('#billcard_'+id).removeClass().addClass(
                            'sno px-3 py-1 font-extrabold text-white text-sm bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 rounded-full  '
                            );
                $('#billcard_'+id).html('<i  class="fa fa-check"></i>');
                selected_bills.push(id);
            }
        //billcard_{id}
            var l=selected_bills.length;
            // console.log(selected_bills);
            if(l>0)
            {
                multi_delete_code='<div class=" font-medium  ">Selected('+selected_bills.length+')&nbsp;&nbsp;<span><i id="multideletebtn" onclick="delete_multi_bill()" class="fa text-red-700 fa-trash"></i></span></div>';
                $('#searchbox').html(multi_delete_code);
            }
            else{
        ...
        function delete_multi_bill()
        {
            const arrayString = JSON.stringify(selected_bills);
            Swal.fire({
                title: 'Are you sure?',
                ...
                confirmButtonText: 'Yes, delete this ('+selected_bills.length+') records'
            }).then((result) => {
                if (result.isConfirmed) {
                    var deleteBill_URL = "{{ config('app.apiurl') }}deleteBills?ids=" + arrayString;
                    // console.log(deleteBill_URL);
                    $.ajax({
                        type: 'GET',
                        url: deleteBill_URL,
                        beforeSend: function() {
                            // Show a loading message or perform other setup actions
                            $('#multideletebtn').removeClass().addClass(
                                'mx-2 fa fa-sync-alt animate-spin text-red-700 font-medium');
                        ...
                        success: function(data) {
                            get_bills();
                            restore_searchbox();
                            selected_bills.splice(0,selected_bills.length);
                            // console.log(data);
                            Swal.fire(
                                'Deleted!',
                                'Your recoeds has been deleted.',
                                'success'
                            )
                        },
                        statusCode: {
                            500: function() {
                                $('#multideletebtn').removeClass().addClass(
                                    'mx-2 fa fa-sync-alt text-red-700 font-medium');
        ...
        function toggleModal() {
            ...
            code +=
                '<div class="fwraper bg-[#eff6ff] py-5 mx-2 mt-2 shadow duration-300 hover:bg-[#dbeafe] hover:shadow-xl rounded-md">\n';
            ...
            code +=
                '<input class="outline-none p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="date" name="date" id="date" required>\n';
            code += '</div>\n';
            ...
            code +=
                '<input  onkeyup="update_pincobde(1,this.value)" type="number" id="from_pincode" name="from_pincode" class="block p-1 w-full text-sm outline-none rounded-r-lg rounded-l-lg border-[1px] border-blue-500 hover:border-blue-700" required>\n';
            code +=
                '<button id="from_pincode_refresh" class="absolute top-0 right-0 p-1 text-sm font-medium text-white bg-blue-700 rounded-r-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">\n';
            code += '<i id="from_pincode_refresh_logo" class="fa fa-redo-alt"></i>\n';
            code += '</button>\n';
            code += '</div>\n';
            code += '</div>\n';
            code +=
                '<select name="from_place" id="from_place" class="outline-none p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700">\n';
            ...
            code += '<div class="relative w-full">\n';
            code +=
                '<input  onkeyup="update_pincobde(2,this.value)" type="number" id="to_pincode" name="to_pincode" class="block p-1 w-full text-sm outline-none rounded-r-lg rounded-l-lg border-[1px] border-blue-500 hover:border-blue-700" required>\n';
            code +=
                '<button  id="to_pincode_refresh" class="absolute top-0 right-0 p-1 text-sm font-medium text-white bg-blue-700 rounded-r-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">\n';
            code += '<i id="to_pincode_refresh_logo" class="fa fa-redo-alt"></i>\n';
            code += '</button>\n';
            code += '</div>\n';
            code += '</div>\n';
            code +=
                '<select name="to_place" id="to_place" class="outline-none p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700">\n';
            code += '<option value=""></option>\n';
            code += '</select>\n';
            code += '</div>\n';
            code += '</div>\n';
            ...
            code +=
                '<label  id="bike_lable" class="rounded px-3 py-2 items-center flex " for="mode_bike"><i class="fa fa-motorcycle"></i></label>\n';
            code += '</div>\n';
            code += '<div class="">\n';
            code += '<input  class="opacity-0" type="radio" id="mode_bus" name="mode" value="bus">\n';
            code +=
                '<label  id="bus_lable" class="rounded px-3 py-2 items-center flex" for="mode_bus"><i class="fa fa-bus"></i></label>\n';
            code += '</div>\n';
            code += '</div>\n';
            code += '</div>\n';
            code += '<div class="flex-cols mt-1 w-full">\n';
            ...
            code +=
                '<input class="outline-none block w-full p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="number" name="km" id="km">\n';
            code += '</div>\n';
            code += '<div class="flex-cols mt-1">\n';
            code += '<div class="">Enter DA</div>\n';
            code +=
                '<input class="outline-none p-1 block w-full rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="number" name="da" id="da">\n';
            code += '</div>\n';
            code += '<div class="flex-cols mt-1">\n';
            code += '<div class="">Enter Amount</div>\n';
            code +=
                '<input class="outline-none p-1 block w-full rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="number" name="amount" id="amount">\n';
            code += '</div>\n';
            code += '</div>\n';
            ...
            code +=
                '<button onClick="adding_done()" class="text-white right-2.5 bottom-2.5 bg-gray-400 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2"> Cancle </button>\n';
            code +=
                '<button id="addBillBtn" onClick="add_bill()" class="text-white right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2"> Add </button>\n';
            code += '</div>\n';
            code += '</div>\n';
            code += '</div>\n';
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data[0]['Status'] == "Success") {
                            var sel_logo = host == 1 ? '#from_pincode_refresh_logo' :
                            '#to_pincode_refresh_logo';
                            // console.log(sel)
                            ...
                            var village = "";
                            for (i = 0; i < count; i++) {
                                village += "<option value=" + obj2[i]['Name'] + ">" + obj2[i]['Name'] +
                                    "</option>";
                            }
        ...
        function add_bill() {
            ...
            if (date == "" || from_pincode == "" || to_pincode == "" || from_place == "" || to_place == "" || mode ==
                undefined || mode == "" || mode == null || km == "" || amount == "") {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        $('#addBillBtn').removeClass().addClass(
                            'inline-flex items-center justify-center w-auto px-3 py-2 space-x-2 text-sm font-medium text-white transition bg-orange-700 border border-orange-700 rounded appearance-none cursor-pointer select-none hover:border-orange-800 hover:bg-orange-800 focus:border-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-300 disabled:pointer-events-none disabled:opacity-75'
                            );
                        $('#addBillBtn').html(
                            '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 animate-spin" viewBox="0 0 20 20" fill="currentColor"> <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" /></svg><span>Adding</span>'
                            );
                    },
                    success: function(data) {
                        // console.log(data);
                        ...
                        if (data["status"] == "success") {
                            ...
                            $('#addBillBtn').removeClass().addClass(
                                'inline-flex items-center justify-center w-auto px-3 py-2 space-x-2 text-sm font-medium text-white transition bg-green-700 border border-green-700 rounded appearance-none cursor-pointer select-none hover:border-green-800 hover:bg-green-800 focus:border-green-300 focus:outline-none focus:ring-2 focus:ring-green-300 disabled:pointer-events-none disabled:opacity-75'
                                );
                            $('#addBillBtn').html('<i class="fa fa-check"></i><span>Done</span>');

                            setTimeout(function() {
                                $('#addBillBtn').removeClass().addClass(
                                    'text-white right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2'
                                    );
                                $('#addBillBtn').html('Add');
                            }, 3000);
                        } else if (data["status"] == "warning") {
                            $('#addBillBtn').removeClass().addClass(
                                'inline-flex items-center justify-center w-auto px-3 py-2 space-x-2 text-sm font-medium text-white transition bg-yellow-700 border border-yellow-700 rounded appearance-none cursor-pointer select-none hover:border-yellow-800 hover:bg-yellow-800 focus:border-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-300 disabled:pointer-events-none disabled:opacity-75'
                                );
                            $('#addBillBtn').html('<i class="fa fa-exclamation"></i><span>Retry</span>');
                        } else if (data["status"] == "error") {
                            $('#addBillBtn').removeClass().addClass(
                                'inline-flex items-center justify-center w-auto px-3 py-2 space-x-2 text-sm font-medium text-white transition bg-red-700 border border-red-700 rounded appearance-none cursor-pointer select-none hover:border-red-800 hover:bg-red-800 focus:border-red-300 focus:outline-none focus:ring-2 focus:ring-red-300 disabled:pointer-events-none disabled:opacity-75'
                                );
                    ...
                    statusCode: {
                        500: function() {
                            ...
                            $('#addBillBtn').html(
                                '<i class="fa fa-exclamation"></i></i><span>Server Error Retry</span>');
                        }
                    }
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data["status"] == "success") {
                            ...
                            $('#editbtn_' + id).removeClass().addClass(
                                'mx-2 fa fa-pen text-green-700 font-medium');
                            adding_done();
                        } else if (data["status"] == "warning") {
                            $('#updateBillBtn').removeClass().addClass(
                                'inline-flex items-center justify-center w-auto px-3 py-2 space-x-2 text-sm font-medium text-white transition bg-yellow-700 border border-yellow-700 rounded appearance-none cursor-pointer select-none hover:border-yellow-800 hover:bg-yellow-800 focus:border-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-300 disabled:pointer-events-none disabled:opacity-75'
                            ...
                            $('#updateBillBtn').html(
                                '<i class="fa fa-exclamation"></i><span> Error Retry</span>');
        ...
        function edit_bill(anthanaistam) {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    // Show a loading message or perform other setup actions
                    $('#editbtn_' + anthanaistam).removeClass().addClass(
                        'mx-2 fa fa-sync-alt animate-spin text-blue-700 font-medium');
                    // $('#editbtn_'+anthanaistam).html('<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 animate-spin" viewBox="0 0 20 20" fill="currentColor"> <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" /></svg><span>Adding</span>');
                },
                success: function(data) {
                    // console.log(data);
                    ...
                    edit_model_code +=
                        '<div class="fwraper bg-[#eff6ff] py-5 mx-2 mt-2 shadow duration-300 hover:bg-[#dbeafe] hover:shadow-xl rounded-md">\n';
                    edit_model_code +=
                        '<div class="fhead rounded-sm flex justify-center items-center text-white bg-blue-700">\n';
                    ...
                    edit_model_code +=
                        '<input class="outline-none p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="date" name="date" id="date" value="' +
                        data["data"].bdate + '" required>\n';
                    edit_model_code += '</div>\n';
                    ...
                    edit_model_code += '<input value="' + data["data"].bfrompincode +
                        '"  onkeyup="update_pincobde(1,this.value)" type="number" id="from_pincode" name="from_pincode" class="block p-1 w-full text-sm outline-none rounded-r-lg rounded-l-lg border-[1px] border-blue-500 hover:border-blue-700" required>\n';
                    edit_model_code +=
                        '<button id="from_pincode_refresh" class="absolute top-0 right-0 p-1 text-sm font-medium text-white bg-blue-700 rounded-r-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">\n';
                    ...
                    edit_model_code +=
                        '<select name="from_place" id="from_place" class="outline-none p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700">\n';
                    edit_model_code += '<option value="' + data["data"].bfromplace + '" selected >' + data[
                        "data"].bfromplace + '</option>\n';
                    ...
                    edit_model_code +=
                        '<input type="radio" name="to_mode" id="to_mode" checked required></div>\n';
                    edit_model_code += '<div class="">\n';
                    edit_model_code += '<label for="to_mode">Name&nbsp</label>\n';
                    edit_model_code +=
                        '<input type="radio" name="to_mode" id="to_mode" disabled required></div>\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '<div class="flex gap-y-1 flex-col">\n';
                    edit_model_code += '<div class="flex">\n';
                    edit_model_code += '<div class="relative w-full">\n';
                    edit_model_code += '<input value="' + data["data"].btopincode +
                        '" onkeyup="update_pincobde(2,this.value)" type="number" id="to_pincode" name="to_pincode" class="block p-1 w-full text-sm outline-none rounded-r-lg rounded-l-lg border-[1px] border-blue-500 hover:border-blue-700" required>\n';
                    edit_model_code +=
                        '<button  id="to_pincode_refresh" class="absolute top-0 right-0 p-1 text-sm font-medium text-white bg-blue-700 rounded-r-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">\n';
                    ...
                    edit_model_code +=
                        '<select name="to_place" id="to_place" class="outline-none p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700">\n';
                    edit_model_code += '<option value="' + data["data"].btoplace + '" selected>' + data["data"]
                        .btoplace + '</option>\n';
                    edit_model_code += '</select>\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '</div>\n';
                    ...
                    if (data["data"].bmode == "bike") {
                        edit_model_code += '<div class="">\n';
                        edit_model_code +=
                            '<input  class="opacity-0" type="radio" id="mode_bike" name="mode" value="bike" checked>\n';
                        edit_model_code +=
                            '<label  id="bike_lable" class="rounded px-3 py-2 items-center flex " for="mode_bike"><i class="fa fa-motorcycle"></i></label>\n';
                        edit_model_code += '</div>\n';
                        ...
                        edit_model_code += '<div class="">\n';
                        edit_model_code +=
                            '<input  class="opacity-0" type="radio" id="mode_bus" name="mode" value="bus">\n';
                        edit_model_code +=
                            '<label  id="bus_lable" class="rounded px-3 py-2 items-center flex" for="mode_bus"><i class="fa fa-bus"></i></label>\n';
                        edit_model_code += '</div>\n';
                        ...
                        edit_model_code += '<div class="">\n';
                        edit_model_code +=
                            '<input  class="opacity-0" type="radio" id="mode_bike" name="mode" value="bike" >\n';
                        edit_model_code +=
                            '<label  id="bike_lable" class="rounded px-3 py-2 items-center flex " for="mode_bike"><i class="fa fa-motorcycle"></i></label>\n';
                        edit_model_code += '</div>\n';
                        ...
                        edit_model_code += '<div class="">\n';
                        edit_model_code +=
                            '<input  class="opacity-0" type="radio" id="mode_bus" name="mode" value="bus" checked>\n';
                        edit_model_code +=
                            '<label  id="bus_lable" class="rounded px-3 py-2 items-center flex" for="mode_bus"><i class="fa fa-bus"></i></label>\n';
                        edit_model_code += '</div>\n';
                    } else {
                    ...
                    edit_model_code += '<input value="' + data["data"].bkm +
                        '" class="outline-none block w-full p-1 rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="number" name="km" id="km">\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '<div class="flex-cols mt-1">\n';
                    edit_model_code += '<div class="">Enter DA</div>\n';
                    edit_model_code += '<input value="' + data["data"].bda +
                        '" class="outline-none p-1 block w-full rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="number" name="da" id="da">\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '<div class="flex-cols mt-1">\n';
                    edit_model_code += '<div class="">Enter Amount</div>\n';
                    edit_model_code += '<input value="' + data["data"].bamount +
                        '" class="outline-none p-1 block w-full rounded-md border-[1px] border-blue-500 hover:border-blue-700" type="number" name="amount" id="amount">\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '<div class="ffoter pt-3">\n';
                    edit_model_code +=
                        '<div id="model_error" class="bg-red-700 rounded shadow text-white font-bold m-2" ></div>\n';
                    edit_model_code += '<div class="flex justify-between px-5">\n';
                    edit_model_code +=
                        '<button onClick="adding_done()" class="text-white right-2.5 bottom-2.5 bg-gray-400 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2"> Cancle </button>\n';
                    edit_model_code += '<button id="updateBillBtn" onClick="update_bill(\'' + anthanaistam +
                        '\')" class="text-white right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2"> Update </button>\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '</div>\n';
                    edit_model_code += '</div>\n';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function() {
                            // Show a loading message or perform other setup actions
                            $('#deletebtn_' + anthanaistam).removeClass().addClass(
                                'mx-2 fa fa-sync-alt animate-spin text-blue-700 font-medium');
                            // $('#editbtn_'+anthanaistam).html('<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 animate-spin" viewBox="0 0 20 20" fill="currentColor"> <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" /></svg><span>Adding</span>');
                        },
                        success: function(data) {
        ...
        function add_clime() {
            Swal.fire({
                ...
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, Submit Clime!'
            }).then((result) => {
                if (result.isConfirmed) {
                    var deleteBill_URL = "{{ config('app.apiurl') }}addClime";
                    $.ajax({
                        type: 'GET',
                        url: deleteBill_URL,
                        beforeSend: function() {
                            $('#addClimeBtn').removeClass().addClass(
                            'inline-flex items-center justify-center w-auto px-3 py-2 space-x-2 text-sm font-medium text-white transition bg-orange-700 border border-orange-700 rounded appearance-none cursor-pointer select-none hover:border-orange-800 hover:bg-orange-800 focus:border-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-300 disabled:pointer-events-none disabled:opacity-75'
                            );
                            $('#addClimeBtn').html(
                            '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 animate-spin" viewBox="0 0 20 20" fill="currentColor"> <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" /></svg><span>Submitting..</span>'
                            );
                        },
                        success: function(data) {
                            ...
                            $('#addClimeBtn').html('<i class="fa fa-check"></i><span>Done</span>');

                            setTimeout(function() {
                                $('#addClimeBtn').removeClass().addClass(
                                    'text-white p-3  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm dark1:bg-blue-600 dark1:hover:bg-blue-700 dark1:focus:ring-blue-800'
                                    );
                                $('#addClimeBtn').html('Submit To Manager');
                            }, 3000);
                                ...
                                $('#addClimeBtn').html('<i class="fa fa-exclamation"></i><span>Retry</span>');
                            } else if (data["status"] == "error") {
                                NormalTost(data["status"],data["message"]);
                                $('#addClimeBtn').removeClass().addClass(
                                    'inline-flex items-center justify-center w-auto px-3 py-2 space-x-2 text-sm font-medium text-white transition bg-red-700 border border-red-700 rounded appearance-none cursor-pointer select-none hover:border-red-800 hover:bg-red-800 focus:border-red-300 focus:outline-none focus:ring-2 focus:ring-red-300 disabled:pointer-events-none disabled:opacity-75'
                                    );
                                $('#addClimeBtn').html('<i class="fa fa-exclamation"></i><span> Error Retry</span>');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data["status"]=="success")
                    {
                        ...
                        NormalTost("warning", "Lets Add Bills 🥳");
                        $("#billswraper").html("<b>"+data["message"]+"</b>");
                    }
                },
                statusCode: {
                    500: function() {
